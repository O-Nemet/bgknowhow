...
if ($selectedId) {
    ...
    $stmt->bind_result($selectedId, $name, $type, $text, $blizzardId, $isActive, $artist);
    ...
    <div class="card_wrapper">
        ...
        <div class="card_picture_big2">
            <img src="<?= PICTURE_LOCAL_ANOMALY . $blizzardId . PICTURE_LOCAL_BIG_SUFFIX ?>" alt="The picture of <?= $name ?>">
        </div>
        <div class="card_info">
            <?= $artist ? 'Artist:' : '' ?> <span class="price_font"><?= $artist ?? '' ?></span><br><br>
        </div>
        <div class="card_picture">
            <img src="<?= PICTURE_LOCAL_ANOMALY . $blizzardId . PICTURE_LOCAL_RENDER_SUFFIX_80 ?>" alt="<?= $text ?>">
        </div>
    </div>
